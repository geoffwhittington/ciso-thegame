...
export const SECTION_HELP: Record<SectionHelpId, SectionHelpCopy> = {
  ...
  advisor: {
    ...
    interpret: 'Up to three items the model can place on the current queue, given visibility and budget. This is a simulation output, not a prescribed program.',
  ...
  systems: {
    ...
    act: 'Anticipate risks discloses specific gaps. Matching control names add that investment to the quarterly queue.',
    topic: 'pipeline',
  },
  investments: {
    title: 'Security investments',
    interpret: 'Add commits a level at quarter close. Remove cancels the last commitment. Markers: owned, pending this quarter, aligned with Anticipate and Execute, or out of scope for current systems.',
    act: 'Controls apply generically across products. Anticipate risks discloses gaps. Execute controls lets staff close listed gaps. Setup is charged this quarter; operating cost continues while the level is held.',
    topic: 'guided',
  },
  endQuarter: {
    title: 'Close the quarter',
    interpret: 'Commitments become live, incidents are applied to production systems, and staff close listed gaps if Execute controls and capacity exist.',
