    ...
    return newsItems.map(news => {
      ...
      switch (news.effect) {
        case 'test_weakness': {
          ...
          if (vulns.length === 0) {
            impact = 'Does this hit NovaMind? No.';
            game.reputation = Math.min(100, game.reputation + 2);
          } else {
            impact = `Does this hit NovaMind? Yes — ${vulns.map((p: any) => p.name).join(', ')}.`;
            game.reputation = Math.max(0, game.reputation - vulns.length);
          }
          break;
        }
        case 'compliance_pressure':
          if ((game.defenses.grc || 0) >= news.value) { impact = 'Does this hit NovaMind? You already meet this bar.'; game.reputation = Math.min(100, game.reputation + 2); }
          else { impact = 'Does this hit NovaMind? Yes — GRC is not ready.'; game.reputation = Math.max(0, game.reputation - 3); }
          break;
        case 'framework_boost': case 'tool_available': impact = 'Industry guidance — extra budget this quarter.'; game.treasury += 75; break;
        ...
        case 'cost_increase': impact = 'Industry-wide: your operating costs go up.'; game.treasury -= 75; break;
        case 'tool_spotlight':
